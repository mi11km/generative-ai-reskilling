...
class TestRAGServiceConversationHistory:
    ...
    def mock_dependencies(self, mock_settings):
        """依存関係のモックを設定"""
        with patch("src.services.rag_service.EmbeddingService") as mock_embedding_service, \
             ...
             patch("src.services.rag_service.SessionService") as mock_session_service_class, \
    ...
    def test_chat_with_new_session(self, mock_dependencies):
        ...
        # 検索結果のモック
        test_search_results = [
            (Document(page_content="テスト内容", metadata={"section": "セクション"}), 0.8)
        ]
        ...
        session_service.get_conversation_history.assert_called_once_with("new-session-id", limit=20)
    ...
    def test_chat_with_existing_session_and_history(self, mock_dependencies):
        ...
        with patch.object(service, "search") as mock_search:
            ...
            with patch.object(service, "generate_answer_with_history") as mock_generate_history:
                mock_generate_history.return_value = "履歴を考慮した回答"
                
                result = service.chat("フォローアップの質問", session_id="existing-session")
        
        # 会話履歴が取得されたことを確認
        session_service.get_conversation_history.assert_called_once_with("existing-session", limit=20)
    ...
    def test_chat_with_session_not_found(self, mock_dependencies):
        ...
        session_service.add_message.side_effect = [None, {"id": "msg-1"}, {"id": "msg-2"}]  # 3回目の呼び出し用も追加
        session_service.create_session.return_value = {"id": "new-session-id"}
        session_service.get_conversation_history.return_value = []
    ...
    def test_generate_answer_with_history(self, mock_dependencies):
        ...
        with patch("src.services.rag_service.LLMChain") as mock_llm_chain:
            ...
            result = service.generate_answer_with_history(query, context_documents, conversation_history)
    ...
    def test_generate_answer_with_long_history(self, mock_dependencies):
        ...
        with patch("src.services.rag_service.ChatPromptTemplate") as mock_prompt_template:
            # from_messagesをモック
            def capture_messages(messages):
                nonlocal captured_system_prompt
                captured_system_prompt = messages[0][1]  # システムプロンプトをキャプチャ
                return Mock()
            ...
            with patch("src.services.rag_service.LLMChain") as mock_llm_chain:
                ...
                service.generate_answer_with_history(query, context_documents, conversation_history)
    ...
    def test_chat_conversation_flow(self, mock_dependencies):
        ...
        test_search_results = [
            (Document(page_content="ゲームの基本ルール", metadata={"section": "ルール"}), 0.9)
        ]
        ...
        test_search_results2 = [
            (Document(page_content="勝利条件の詳細", metadata={"section": "勝利条件"}), 0.85)
        ]
        ...
        with patch.object(service, "search") as mock_search:
            ...
            with patch.object(service, "generate_answer_with_history") as mock_generate_history:
                mock_generate_history.return_value = "先ほど説明したルールに加えて、勝利条件は..."
                
                result2 = service.chat("勝利条件についてもう少し詳しく", session_id=session_id)
