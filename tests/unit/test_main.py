...
class TestLifespan:
    ...
    async def test_lifespan_successful_startup(self, mock_logger, mock_rag_service, mock_get_settings):
        """正常な起動プロセスをテスト"""
        mock_settings = Mock()
        mock_get_settings.return_value = mock_settings
...
class TestMain:
    ...
    @patch("src.main.get_settings")
    @patch("src.main.logger")
    def test_main_successful_execution(self, mock_logger, mock_get_settings, mock_create_app, mock_uvicorn_run):
        """正常なメイン関数の実行をテスト"""
        mock_settings = Mock()
        ...
        # 起動ログが出力されることを確認
        mock_logger.info.assert_called_with("サーバーを起動します: http://localhost:8000")
    
    @patch("src.main.uvicorn.run")
    @patch("src.main.create_app")
    @patch("src.main.get_settings")
    @patch("src.main.logger")
    def test_main_with_debug_mode(self, mock_logger, mock_get_settings, mock_create_app, mock_uvicorn_run):
...
class TestMainIntegration:
    ...
    @patch("src.main.RAGService")
    @patch("src.main.get_settings")
    def test_create_app_with_lifespan_integration(self, mock_get_settings, mock_rag_service, mock_uvicorn_run):
...
@pytest.mark.parametrize("debug_mode,expected_reload", [
    (True, True),
    (False, False),
])
@patch("src.main.uvicorn.run")
@patch("src.main.create_app")
@patch("src.main.get_settings")
def test_main_debug_mode_parameter(mock_get_settings, mock_create_app, mock_uvicorn_run, debug_mode, expected_reload):
