...
from src.models.schemas import ChatRequest, ChatResponse, SourceDocument, HealthResponse
...
class TestSourceDocument:
    ...
    def test_valid_source_document(self):
        ...
        doc = SourceDocument(content="ドキュメントの内容", section="セクション名")
    ...
    def test_source_document_with_metadata(self):
        ...
        doc = SourceDocument(content="内容", section="セクション", metadata=metadata)
...
class TestChatResponse:
    ...
    def test_valid_chat_response(self):
        ...
        response = ChatResponse(answer="回答です", sources=sources, confidence=0.85)
    ...
    def test_chat_response_empty_sources(self):
        ...
        response = ChatResponse(answer="回答", sources=[], confidence=0.0)
    ...
    def test_confidence_boundary_values(self):
        ...
        # 最大値 (1.0)
        response = ChatResponse(answer="回答", sources=[], confidence=1.0)
    ...
    def test_confidence_invalid_values(self):
        ...
        # 0.0未満
        with pytest.raises(ValidationError):
            ChatResponse(answer="回答", sources=[source], confidence=-0.1)

        # 1.0より大きい
        with pytest.raises(ValidationError):
            ChatResponse(answer="回答", sources=[source], confidence=1.1)
...
class TestHealthResponse:
    ...
    def test_valid_health_response(self):
        ...
        response = HealthResponse(
            status="healthy", version="0.1.0", vector_store_ready=True
        )
    ...
    def test_health_response_unhealthy_status(self):
        ...
        response = HealthResponse(
            status="unhealthy", version="0.1.0", vector_store_ready=False
        )
...
def test_confidence_parameter_values(confidence, expected):
    ...
    source = SourceDocument(content="内容", section="セクション")
    response = ChatResponse(answer="回答", sources=[source], confidence=confidence)
