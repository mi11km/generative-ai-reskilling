...
class TestEmbeddingService:
    ...
    def test_japanese_text_embedding(self, mock_huggingface_embeddings):
        ...
        japanese_text = (
            "これは日本語のテストテキストです。ゲーム仕様について説明します。"
        )
    ...
    def test_logging_during_initialization(
        self, mock_logger, mock_huggingface_embeddings
    ):
        """初期化時のログ出力をテスト"""
        mock_embeddings_instance = Mock()
        mock_huggingface_embeddings.return_value = mock_embeddings_instance
        ...
        mock_logger.info.assert_any_call(
            "埋め込みモデルを初期化中: intfloat/multilingual-e5-large"
        )
...
@patch("src.services.embeddings.HuggingFaceEmbeddings")
def test_embed_documents_various_lengths(
    mock_huggingface_embeddings, texts, expected_length
):
