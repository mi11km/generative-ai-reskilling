...
describe('ChatInterface', () => {
  ...
  it('初期状態で正しく表示される', () => {
    ...
    expect(screen.getByText('ゲーム仕様について何でもお聞きください')).toBeInTheDocument();
    expect(screen.getByText('ゲーム仕様について質問してみましょう')).toBeInTheDocument();
    expect(screen.getByPlaceholderText('ゲーム仕様について質問してください...')).toBeInTheDocument();
  });
  ...
  it('メッセージを送信できる', async () => {
    ...
    const input = screen.getByPlaceholderText('ゲーム仕様について質問してください...');
  ...
  it('ローディング中は送信ボタンが無効になる', async () => {
    ...
    const submitBtn = buttons.find(button => button.getAttribute('type') === 'submit');
  ...
  it('APIエラーを適切に表示する', async () => {
    ...
    mockApiClient.chat.mockRejectedValueOnce(new Error('APIエラーが発生しました'));
    ...
    await waitFor(() => {
      expect(screen.queryByText('APIエラーが発生しました')).not.toBeInTheDocument();
    });
  });
