...
class TestAPIIntegration:
    ...
    def test_full_chat_api_flow(self, app, client):
        ...
        try:
            ...
            assert (
                data["answer"]
                == "スゲリス・サーガは冒険RPGゲームです。プレイヤーは勇者となり、魔王を倒すことが最終目標となります。"
            )
    ...
    def test_chat_api_validation_integration(self, client):
        ...
        response = client.post(
            "/api/v1/chat", json={"question": "テスト質問", "max_results": 15}
        )
        assert response.status_code == 422
    ...
    def test_multiple_concurrent_requests_integration(self, app, client):
        ...
        def mock_chat_response(question, max_results, session_id=None):
            # 少し遅延を追加してコンカレンシーをテスト
            time.sleep(0.1)
            return ChatResponse(answer=f"回答: {question}", sources=[], confidence=0.5)
        ...
        def make_request(question_id):
            response = client.post(
                "/api/v1/chat", json={"question": f"テスト質問{question_id}"}
            )
    ...
    def test_no_results_scenario_integration(self, app, client):
        ...
        try:
            # 見つからない内容でのリクエスト
            response = client.post(
                "/api/v1/chat", json={"question": "存在しない機能についての質問"}
            )
    ...
    def test_large_response_handling_integration(self, app, client):
        ...
        large_response = ChatResponse(
            answer=large_answer, sources=large_sources, confidence=0.75
        )
        ...
        try:
            ...
            response = client.post(
                "/api/v1/chat",
                json={"question": "詳細な情報を教えて", "max_results": 5},
            )
...
class TestAPIWithRealDependencies:
    ...
    def test_full_stack_integration(self, app_with_real_dependencies):
        ...
        # 実際のAPIコールをテスト
        response = client.post(
            "/api/v1/chat", json={"question": "ゲーム概要について教えて"}
        )
