...
vi.mock('../components/SourceInfo', () => ({
  SourceInfo: ({ sources, confidence }: { sources: { length: number }; confidence: number }) => (
...
describe('MessageBubble', () => {
  ...
  it('アシスタントメッセージを正しく表示する', () => {
    ...
    expect(screen.getByText('こんにちは！お手伝いできることがありますか？')).toBeInTheDocument();
  ...
  it('ユーザーメッセージには青い背景が適用される', () => {
    ...
    const messageElement = screen.getByText('こんにちは、テストメッセージです').parentElement;
  ...
  it('アシスタントメッセージには白い背景が適用される', () => {
    ...
    const messageElement = screen.getByText('こんにちは！お手伝いできることがありますか？').parentElement;
  ...
  it('アシスタントメッセージのソース情報が含まれることを確認', () => {
    ...
    expect(screen.getByText('Sources: 1, Confidence: 0.95')).toBeInTheDocument();
  });
