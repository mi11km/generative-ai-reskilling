...
class TestFullRAGPipeline:
    ...
    def test_search_to_answer_generation_pipeline(
        ...
        mock_vector_store.similarity_search_with_score.return_value = test_search_results
        ...
        with patch("src.services.rag_service.LLMChain") as mock_llm_chain:
            ...
            mock_chain_instance.run.return_value = "スゲリス・サーガは冒険RPGで、ターン制バトルシステムを使用しています。"
            ...
            # チャット機能のテスト
            response = rag_service.chat("ゲームの基本システムについて教えて", max_results=2)
            
            # 結果の検証
            assert isinstance(response, ChatResponse)
            assert response.answer == "スゲリス・サーガは冒険RPGで、ターン制バトルシステムを使用しています。"
    ...
    def test_similarity_threshold_filtering_integration(
        ...
        test_search_results = [
            (Document(page_content="高関連性コンテンツ", metadata={"section": "セクション1"}), 0.8),
            (Document(page_content="中関連性コンテンツ", metadata={"section": "セクション2"}), 0.6),
            (Document(page_content="低関連性コンテンツ", metadata={"section": "セクション3"}), 0.3),  # しきい値以下
            (Document(page_content="最低関連性コンテンツ", metadata={"section": "セクション4"}), 0.1),  # しきい値以下
        ]
        mock_vector_store.similarity_search_with_score.return_value = test_search_results
        ...
        with patch("src.services.rag_service.LLMChain") as mock_llm_chain:
            mock_chain_instance = Mock()
            mock_llm_chain.return_value = mock_chain_instance
            mock_chain_instance.run.return_value = "フィルタリングされた結果に基づく回答"
    ...
    def test_context_length_truncation_integration(
        ...
        with patch("src.services.rag_service.LLMChain") as mock_llm_chain:
            mock_chain_instance = Mock()
            mock_llm_chain.return_value = mock_chain_instance
            mock_chain_instance.run.return_value = "切り詰められたコンテキストからの回答"
            ...
            answer = rag_service.generate_answer("質問", context_documents)
            ...
            assert len(context) <= integration_settings.max_context_length + 3  # "..."を含む
    ...
    def test_multiple_chunks_from_same_section_integration(
        ...
        with patch("src.services.rag_service.LLMChain") as mock_llm_chain:
            mock_chain_instance = Mock()
            mock_llm_chain.return_value = mock_chain_instance
            mock_chain_instance.run.return_value = "バトルシステムとスキルシステムについての統合回答"
