...
describe('SessionSidebar', () => {
  ...
  it('現在選択中のセッションがハイライトされる', async () => {
    ...
    await waitFor(() => {
      const sessionElement = screen.getByLabelText('Select session: テストセッション1');
  ...
  it('セッション削除が確認されるとAPIが呼ばれる', async () => {
    vi.mocked(apiClient.getSessions).mockResolvedValue(mockSessions);
    vi.mocked(apiClient.deleteSession).mockResolvedValue({ message: 'Deleted' });
  ...
  it('読み込み中の状態が表示される', async () => {
    vi.mocked(apiClient.getSessions).mockImplementation(() => new Promise(() => {}));
  ...
  it('エラー状態が表示される', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/API Error|セッション一覧の取得に失敗しました/)).toBeInTheDocument();
    });
  });
  ...
  it('時間のフォーマットが正しく表示される', async () => {
    ...
    await waitFor(() => {
      ...
      expect(document.querySelector('.text-sm.text-gray-500')).toBeInTheDocument();
    });
  });
});