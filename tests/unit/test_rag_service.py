...
class TestRAGService:
    ...
    def test_init_successful(self, mock_init, mock_chat_openai, mock_embedding_service, mock_settings):
        """正常な初期化をテスト"""
    ...
    def test_initialize_vector_store_existing_directory(
        self, mock_chroma, mock_exists, mock_chat_openai, mock_embedding_service, mock_settings
    ):
    ...
    def test_initialize_vector_store_new_directory(
        self, mock_create, mock_exists, mock_chat_openai, mock_embedding_service, mock_settings
    ):
    ...
    def test_create_vector_store(
        self, mock_chroma, mock_document_loader, mock_exists, mock_chat_openai, mock_embedding_service, mock_settings
    ):
        """ベクトルストア作成をテスト"""
        mock_exists.return_value = False
        ...
        mock_document_loader.assert_called_once_with(
            "test_spec.md",
            500,
            100
        )
    ...
    @patch("src.services.rag_service.ChatOpenAI")
    @patch("src.services.rag_service.RAGService._initialize_vector_store")
    def test_search_successful(self, mock_init, mock_chat_openai, mock_embedding_service, mock_settings):
        """正常な検索をテスト"""
        service = RAGService(mock_settings)
    ...
    @patch("src.services.rag_service.ChatOpenAI")
    @patch("src.services.rag_service.RAGService._initialize_vector_store")
    def test_search_with_threshold_filtering(self, mock_init, mock_chat_openai, mock_embedding_service, mock_settings):
        ...
        test_documents = [
            (Document(page_content="内容1", metadata={"section": "セクション1"}), 0.8),
            (Document(page_content="内容2", metadata={"section": "セクション2"}), 0.3),  # しきい値以下
        ]
        mock_vector_store.similarity_search_with_score.return_value = test_documents
    ...
    @patch("src.services.rag_service.ChatOpenAI")
    @patch("src.services.rag_service.RAGService._initialize_vector_store")
    def test_search_vector_store_not_initialized(self, mock_init, mock_chat_openai, mock_embedding_service, mock_settings):
    ...
    def test_generate_answer(self, mock_llm_chain, mock_init, mock_chat_openai, mock_embedding_service, mock_settings):
        """回答生成をテスト"""
        service = RAGService(mock_settings)
    ...
    @patch("src.services.rag_service.RAGService._initialize_vector_store")
    @patch("src.services.rag_service.LLMChain")
    def test_generate_answer_long_context(self, mock_llm_chain, mock_init, mock_chat_openai, mock_embedding_service, mock_settings):
        """長いコンテキストの切り詰めをテスト"""
        service = RAGService(mock_settings)
        ...
        result = service.generate_answer("質問", test_documents)
    ...
    @patch("src.services.rag_service.ChatOpenAI")
    @patch("src.services.rag_service.RAGService._initialize_vector_store")
    def test_chat_successful(self, mock_init, mock_chat_openai, mock_embedding_service, mock_settings):
        """正常なチャット処理をテスト"""
        service = RAGService(mock_settings)
        
        # searchメソッドのモック
        test_search_results = [
            (Document(page_content="テスト内容1", metadata={"section": "セクション1"}), 0.8),
            (Document(page_content="テスト内容2", metadata={"section": "セクション2"}), 0.6),
        ]
    ...
    @patch("src.services.rag_service.ChatOpenAI")
    @patch("src.services.rag_service.RAGService._initialize_vector_store")
    def test_chat_no_results(self, mock_init, mock_chat_openai, mock_embedding_service, mock_settings):
    ...
    @patch("src.services.rag_service.ChatOpenAI")
    @patch("src.services.rag_service.RAGService._initialize_vector_store")
    def test_chat_with_truncated_source_content(self, mock_init, mock_chat_openai, mock_embedding_service, mock_settings):
        """長いソース内容の切り詰めをテスト"""
        service = RAGService(mock_settings)
        ...
        test_search_results = [
            (Document(page_content=long_content, metadata={"section": "長いセクション"}), 0.8),
        ]
    ...
    @patch("src.services.rag_service.ChatOpenAI")
    @patch("src.services.rag_service.RAGService._initialize_vector_store")
    def test_is_ready_true(self, mock_init, mock_chat_openai, mock_embedding_service, mock_settings):
    ...
    @patch("src.services.rag_service.ChatOpenAI")
    @patch("src.services.rag_service.RAGService._initialize_vector_store")
    def test_is_ready_false(self, mock_init, mock_chat_openai, mock_embedding_service, mock_settings):
